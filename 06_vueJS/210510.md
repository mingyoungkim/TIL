...
// App.vue
	...
	// router-link 에 :to (이름 넣어주기) => django 에서 url.py 의 urlname과 같은 개념 
		// v-bind 써주고 객체로 표현하기 => '/', 'About' 같은 것을 직접 치지 않아도 됨 (관리 편해짐)
		// => index.js의 routs의 name과 같음

// index.js
	// About 등록 '../' => 위에 위에있는 상위 디렉토리 ('@/' == '/src' 절대경로)
